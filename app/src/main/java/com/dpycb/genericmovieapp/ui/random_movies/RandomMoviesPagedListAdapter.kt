...
class RandomMoviesPagedListAdapter(private val context: Context): PagedListAdapter<Movie, RecyclerView.ViewHolder>(
    RandomMoviesPagedListAdapter.RandomMovieDiffUtil()
) {
    val MOVIE_TYPE = 1
    val NETWORK_TYPE = 2
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        if (viewType == MOVIE_TYPE) {
            ...
            return RandomMovieViewHolder(view)
        }
        else {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == MOVIE_TYPE) {
            (holder as RandomMovieViewHolder).bind(getItem(position), context)
        }
        else {
    ...
    class RandomMovieDiffUtil: DiffUtil.ItemCallback<Movie>() {
    ...
    class RandomMovieViewHolder(view: View): RecyclerView.ViewHolder(view) {
